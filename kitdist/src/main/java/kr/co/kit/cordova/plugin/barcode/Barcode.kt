package kr.co.kit.cordova.plugin.barcode
...
class Barcode : SimpleCordovaPlugin() {
    ...
    override fun execute(args: JSONArray): Boolean = when (action) {
        ACTION_OPEN_BARCODE_SCANNER -> activity.runOnUiThread { startBarcodeScanner() }.run { true }
        else -> false
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, intent: Intent?) {
        when (requestCode) {
            REQ_CODE_BARCODE_SCANNER -> IntentIntegrator.parseActivityResult(
                resultCode, intent
            )?.let {
